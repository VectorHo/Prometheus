...
module.exports.route = function(app, route) {
  ...
  // 修改root密码
  app.use(route.post(api_v1 + '/password', function*() {
  ...
  // 管理员添加用户
  app.use(route.post(api_v1 + '/users', function*() {

  }));

  // 管理员删除用户
  app.use(route.delete(api_v1 + '/users/:user_id', function*(user_id) {

  }));

  // 管理员修改用户
  app.use(route.put(api_v1 + '/users/:user_id', function*(user_id) {

  }));

  // 管理员查询用户
  app.use(route.get(api_v1 + '/users/:user_id', function*(user_id) {
