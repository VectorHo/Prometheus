...
module.exports.route = function(app, route) {
  ...
  app.use(route.get(api_v1 + '/logs', function*() {
    ...
    // 检测输入的是否只为："level", "action", "type", "user", "project", "job", "beginTime", "endTime"，如果不是，则报错
    validator.keyDifference(this, conditions, ["level", "action", "type", "user", "project", "job", "beginTime", "endTime"]);

    // 检测level, action, type 等是否符合要求
    validator.enumValidator(this, "level", ["red", "yellow", "green"]);
    validator.enumValidator(this, "action", ["GET", "HEAD", "POST", "PUT", "DELETE", "OPTIONS", "TRACE"]);
    validator.enumValidator(this, "type", ["user", "project", "job"]);
