...
module.exports.route = function(app, route) {
  ...
  app.use(route.get(api_v1 + '/logs', function*() {
    ...
    var keyDifference = _.difference(_.keys(conditions), ["level", "action", "type", "user", "project", "job", "beginTime", "endTime"]);
    if (0 != keyDifference.length) {
      this.status = 403;
      return this.body = {
        "message": "Only the following options: level, action, type, user, project, job, beginTime, endTime"
      };
    }
